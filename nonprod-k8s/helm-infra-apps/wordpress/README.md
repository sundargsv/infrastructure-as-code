## Setting up a Production Grade Wordpress Instance in Kubernetes connecting with PostgresSQL database

```
helm install wordpress-dev . --values=values-development.yaml --namespace=dev-ui --dry-run --debug
```
```
helm install wordpress-dev  wordpress/. --namespace=dev-ui
```

```
helm uninstall wordpress-dev -n dev-ui
```
